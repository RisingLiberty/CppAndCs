...
void foo()
{
  std::cout << "This is called from C#!!";
}
...
//{
    ...
    int run_app_example(const string_t& root_path)
    {
        ...
        int rc = init_for_cmd_line_fptr(static_cast<int>(args.size()), args.data(), nullptr, &cxt);
        if (rc != 0 || cxt == nullptr)
        {
        ...
        // Call into C#, it will call back into C++
        typedef void (CORECLR_DELEGATE_CALLTYPE* cpptocs_fn)(const char*);
        cpptocs_fn cpptocs;
        rc = get_function_pointer(
          STR("Internal, App"),
          STR("CppToCs"),
          UNMANAGEDCALLERSONLY_METHOD,
          nullptr, nullptr, (void**)&cpptocs
        );
        assert(rc == 0 && cpptocs != nullptr && "Failure: get_function_pointer()");

        // Call into C#, it will call back into C++
        typedef void (CORECLR_DELEGATE_CALLTYPE* setdelegate_fn)(const void*, int);
        setdelegate_fn setdelegate;
        rc = get_function_pointer(
          STR("Internal, App"),
          STR("SetDelegate"),
          UNMANAGEDCALLERSONLY_METHOD,
          nullptr, nullptr, (void**)&setdelegate
        );
        assert(rc == 0 && setdelegate != nullptr && "Failure: get_function_pointer()");
        InternalCall call{};
        call.Name = "foo";
        call.NativeFunctionPtr = foo;
        setdelegate(&call, sizeof(InternalCall));
        cpptocs("Hello from C++");
